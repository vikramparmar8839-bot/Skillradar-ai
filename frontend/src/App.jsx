      ...
      <main className="main-area">
        ...
        <div className="content">
          ...
          <section id="market" className="sih-section page-section">
            ...
            <div className="sih-grid-two"><article className="sih-panel"><div className="card-header"><div><span className="section-kicker">DEMAND ENGINE</span><h3>Top skills by industry demand</h3></div><span className="live-pill">LIVE SIGNAL</span></div><div className="market-skill-list">{marketLoading?<div className="market-loading"><span className="loading-dot"/><span>Loading live market signals…</span></div>:marketSkills.length?marketSkills.map((skill)=><div className="market-skill-row" key={skill.name}><div className="market-skill-meta"><strong>{skill.name}</strong><span>{skill.demand}% demand • {skill.level}</span></div><div className="market-bar"><i style={{width:`${skill.demand}%`}}/></div></div>):<div className="empty-mini">No analysed job postings yet.</div>}</div></article><article className="sih-panel emerging-panel"><div className="card-header"><div><span className="section-kicker">TREND DETECTOR</span><h3>Emerging technology signals</h3></div><span className="source-badge">AI FLAGGED</span></div>{marketSkills.length>0?<><div className="emerging-feature"><div><strong>{marketSkills[0].name}</strong><p>Highest current demand signal in the ingested job-posting dataset.</p></div></div><div className="emerging-list">{marketSkills.slice(1,4).map(skill=><span key={skill.name}>{skill.name}</span>)}</div><div className="recommendation-box"><strong>Evidence-based signal</strong><p>Trend is calculated from recent versus previous posting windows in the backend.</p></div></>:<div className="empty-mini">No analysed job postings yet.</div>}</article></div>
          </section>
