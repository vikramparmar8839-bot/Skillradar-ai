...
function App() {
  ...
  const [courseHealthStatus, setCourseHealthStatus] = useState("Loading curriculum alignment...");
  const trainingPlan = [
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/market/curriculum-alignment`)
      .then((response) => {
        if (!response.ok) throw new Error("Curriculum alignment request failed");
        return response.json();
      })
      .then((data) => {
        setCourseHealth(data.curriculums || []);
        setCourseHealthStatus(
          data.curriculums?.length ? "LIVE MARKET + SYLLABUS" : "NO CURRICULUM DATA"
        );
      })
      .catch((error) => {
        console.log("Error loading curriculum alignment:", error);
        setCourseHealthStatus("UNAVAILABLE");
      });
  }, []);
      ...
      <main className="main-area">
        ...
        <div className="content">
          ...
          <section id="courses" className="sih-section page-section">
            <div className="sih-section-heading"><div><span className="card-kicker">09 / CURRICULUM ALIGNMENT</span><h3>Curriculum vs industry demand</h3><p>Compare your syllabus with the strongest skills observed in the Indian job market.</p></div><span className="source-badge warning">{courseHealthStatus}</span></div>
            <div className="course-table">
              <div className="course-row course-head"><span>CURRICULUM</span><span>ALIGNMENT</span><span>COVERED</span><span>TOP GAPS</span><span>RECOMMENDATION</span></div>
              {courseHealth.map((course) => (
                <div className="course-row" key={course.curriculum}>
                  <strong>{course.curriculum}</strong>
                  <span>{course.alignment}%</span>
                  <span>{course.covered_count}/{course.benchmark_count}</span>
                  <span>{course.gaps?.length ? course.gaps.slice(0, 3).join(", ") : "No major gaps"}</span>
                  <b className={course.status === "WELL ALIGNED" ? "good" : course.status === "UPDATE" ? "warn" : "danger"}>{course.status}</b>
                </div>
              ))}
              {!courseHealth.length && <div className="course-row"><span>Curriculum alignment data is loading...</span></div>}
            </div>
          </section>
